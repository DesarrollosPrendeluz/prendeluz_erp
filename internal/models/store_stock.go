...
type StoreStock struct {
	...
	SKU_Parent     string `gorm:"column:parent_main_sku;not null"`
	Amount         int64  `gorm:"column:quantity"`
	ReservedAmount int64  `gorm:"column:reserved"`
	CreatedAt      time.Time
	UpdatedAt      time.Time

	Item      Item            `gorm:"foreignKey:SKU_Parent;references:MainSKU;constraint:OnUpdate:CASCADE,OnDelete:RESTRICT"`
